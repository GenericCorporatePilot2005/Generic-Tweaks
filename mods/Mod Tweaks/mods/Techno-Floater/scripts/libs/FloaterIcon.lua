...
local function IsFloater(pawn)
	...
	return pawnData and pawnType == "Nico_Techno_Floater"
end
...
local function finalizeInit(self)	
	newicon = sdlext.surface(mod.resourcePath .."img/combat/icons/Nico_icon_floater.png")
	...
	function GetStatusTooltip(id)
		if	id == "flying"		and
			...
			return {"Floater", "This unit can move over any terrain tile, can fly to unoccupied creep."}
		end
...
local function onModsInitialized()
	local isHighestVersion = true
		and FloaterIcon.initialized ~= true
		and FloaterIcon.version == VERSION

	if isHighestVersion then
		FloaterIcon:finalizeInit()
		FloaterIcon.initialized = true
	end
end
...
local isNewerVersion = false
	or FloaterIcon == nil
	or VERSION > FloaterIcon.version

if isNewerVersion then
	FloaterIcon = FloaterIcon or {}
	FloaterIcon.version = VERSION
	FloaterIcon.finalizeInit = finalizeInit
